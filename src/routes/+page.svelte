...
{#if !paused}
	...
	>
		<div class="flex w-full flex-col gap-1 bg-stone-100 px-4">
			<h1 class="py-6 text-3xl font-extralight text-stone-600 lg:text-5xl">
...
{:else}
	<div class="flex h-screen w-screen flex-col items-center justify-center gap-12">
		<h1 class="flex flex-row items-center justify-center text-6xl font-black text-stone-600">
			Site paused 😓
		</h1>
		<small class="flex flex-row items-center justify-center text-md font-medium text-stone-800">
			A lot of hate & troll content was posted , Gimme a moment to clean that up.
		</small>
	</div>
{/if}
